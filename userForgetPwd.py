...
def startReset(m):
	...
	root.title("Reset Password Page") 
	...
	Label(rightFrame,text="Reset Password",width="20", fg="blue", font = "Times 20 bold").pack()
	Label(rightFrame,text="").pack()
	...
	Label(rightFrame,text="").pack()
	Label(rightFrame,text="Enter New Password",width="20",font="Times 14 bold").pack()
	Entry(rightFrame,textvariable=newpassword,font="Times 14 bold",show="*").pack(expand = True, fill = BOTH)
	Label(rightFrame,text="").pack()
	...
	Label(rightFrame,text="").pack()
	ActB=Button(rightFrame,text="Reset Password",fg="black", activebackground = "white",width="20",font="Times 14 bold",command=lambda:ResetPwdB(root,cardno,otp,newpassword,Repassword,time))
	ActB.pack()
	Label(rightFrame,text="").pack()
...
def sendOTP(receiver_email,otp_ran,time,root,ActB,otpB):
	try:
		...
		message["Subject"] = "Voter Card Forget Password"
		message["From"] = sender_email
		message["To"] = receiver_email
		text = """\
		Hi there,
		Your Voter Card Forget Password OTP is:"""+str(otp_ran)+"""\
		...
		<body>
			<p>Hi there,<br>
			Your Voter Card Forget Password OTP is:<br>"""+str(otp_ran)+"""\
			Don't share this otp with anyone else.
			</p>
		</body>
		</html>
	...
	return
def ResetPwdB(root,cardno,otp,password,Repassword,time):
	...
	if(len(cardnoS)!=10):
		...
		return
	if not (db.isVoterAccountAlreadyActivated(cardnoS)):
		messagebox.showerror("Error","This Voter card id Account is not Activated")
		return
	if(len(passwordS)!=6):
	...
	if(otpSInt!=otp_ran):
		...
		return
	db.ResetPwdActiveVoter(cardnoS,passwordS)
	messagebox.showinfo("Done","Password Reseted!")
	back_bu(root)
